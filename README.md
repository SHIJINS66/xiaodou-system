...
OpenClaw 是生成式、响应式的：它会回应，但不会主动、定时、自主地经营一天；它依赖模型的偶发输出，缺少强确定性的运维保障。这正是陪伴系统缺失、而本框架补齐的部分。我们实际改造中发现了 OpenClaw 的几个明显不足，也针对性地做了系统性调整：

- **主动性不足：heartbeat 受限且不可控。** OpenClaw 自带的 heartbeat 心跳机制只能依附于单个 session 唤醒模型，既不跨会话，时机也不可控——它没法在指定时刻、以可预期的节奏主动触达用户。因此本框架放弃了依赖 heartbeat 驱动主动行为，改用操作系统级的 `cron`（固定时间触发）与 `at`（一次性定时）来编排 step02 / step03 / step04，让主动性从「模型偶然醒来」变成「确定性的系统节律」；
- **主动性（Proactivity）**：在定时调度之上，本框架补出 step02 每日规划器（读取核心文件与天气生成全天轨迹），并通过 step03 将事件提交到系统级 `at` 定时器，实现了真正的自主触达；
...
- **记忆无条理、无限膨胀：memory 与 dream 机制改进了前提，却不收敛。** OpenClaw 的 memory 写入与 dream（梦境）回顾有一定作用，但它长期把记忆写进同一个 `MEMORY.md`，文件只会单调膨胀、越积越乱，缺少组织与持久收敛。本框架的 step04 用确定性脚本接管这一过程：把记忆分成「当日融合记忆（daily）→ 增量并入 → 每周 LLM 去重压缩重写」的多层结构，并将正文篇幅长期收敛于上限（≤5000 字），让长期记忆不再是无限堆积的档案，而是有条理、可持续增长的资产；
- **主动消息的注入可见性：让模型看得见自己说过什么。** OpenClaw 的主动消息注入（`chat.inject`）默认未必把「角色自己刚发送的消息」放回会话上下文；若不处理，模型会在被唤醒时看不见自己之前的主动触达，导致记忆与行为脱节。本框架在同一条注入链路里，把实际发送的配文、事件类型、场景、图片路径等一并写回会话历史，让模型下次醒来时能完整看到自己主动说了什么、出于什么场景，保持言行与记忆的连贯；
