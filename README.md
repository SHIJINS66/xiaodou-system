...
一个角色的一天，框架会自动安排好整条时间线：

```text
06:00  ☀️  晨间 Planner
        └─ 读你的核心文件(AGENTS/SOUL/…) → 天气 → 生成当天完整生活轨迹
           (几点起床 / 穿什么 / 吃什么 / 何时发你消息 / 开心还是累)
           ↓  JSON schema 校验，不合格就重试，绝不硬凑

06:20  📅  调度(at)
        └─ 把当天所有“非静默”事件(聊聊天/发张自拍)排序，交给 at 定时

09:14  💬  哎，今天地铁好挤，想起你了 → (到点自动触达你)
12:30  🍜  午饭时间，顺手给你发一张
18:20  📸  下班，拍张自拍发你
        └─ 每次:先看你现在忙不忙 → 生成恰好的文案 → 真正发出去 → 记进当日档案

00:20  🌙  日终记忆(finalize)
        └─ 把今天的聊天 + 你发的消息 + 日常，融成一段“今天过得怎样”的记忆

02:00  📝  长期记忆(update)
        └─ 挑出值得长期记住的 → 并进 MEMORY.md(保留最近 7 天，超额自动压缩)

04:00  🔄  会话重置(rollover)
        └─ 把昨晚的对话带到今天 + 重置会话，新的一天重新开始
```

> 每一步都是**确定性脚本 + JSON schema + 幂等门 + flock 锁**，不是靠模型“大概跑一下”。
> 那句「哎今天地铁好挤」是举例——角色的语气、话题、频率由 `SOUL.md` 和当天的
> 生活轨迹决定，框架只管把它在**对的时间、以对的方式**送到你面前。
